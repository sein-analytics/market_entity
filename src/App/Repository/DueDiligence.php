...
class DueDiligence extends DueDiligenceAbstract
{
    ...
    private string $fetchDueDiligenceByIdSql = "SELECT dd.*, user.issuer_id FROM DueDiligence AS dd LEFT JOIN MarketUser AS user ON user.id = dd.user_id WHERE dd.id=?;";
    ...
    public function fetchDueDilBuyersDealLoans(int $dealId, int $userId, ?string $attrsFilter = null )
    {
        return $this->executeProcedure([ $dealId, $userId, $attrsFilter ], self::$callDueDilBuyersDealLoans);
    }
