    ...
    implements DbalStatementInterface, IssueInterface, DueDiligenceInterface, DueDilLoanStatusInterface
{
    ...
    protected static string $dueDiligenceTeamDataSql = 'SELECT id AS userId, concat(first_name,\' \', last_name ) AS teamMemberName, role_id AS roleId, image_arn AS imageLink FROM user_name AS email `MarketUser` WHERE issuer_id = ? AND id != ?';
